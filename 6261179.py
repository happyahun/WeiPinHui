...
train_goods.info(), train_user.info()
train_goods.isnull().sum(), train_user.isnull().sum()
...
train_data['weight'] = train_data['is_clk']*1 + train_data['is_like']*2.17 + train_data['is_addcart']*5.19 + train_data['is_order']*15.65
train_data['weight'].astype(int)
...
train_data['time'] = train_data['expose_start_time'].apply(lambda x: int(x[11:13]))
train_data.loc[train_data['time']<=7, 'time'] = -1
train_data.loc[(train_data['time']<=19) & (train_data['time']>=8) , 'time'] = 1
train_data.loc[(train_data['time']<=23) & (train_data['time']>=20) , 'time'] = -1
train_data['time']
...
train_data['dt'] = train_data['dt']%100
import math
train_data['dt'] = train_data['dt'].apply(lambda x: math.exp((30-x)/7* math.log(0.5,math.e) ))
...
corr['is_order'].sort_values(ascending=False)


# In[23]:


train_uesr1 = train_data.iloc[:].groupby(['user_id']).agg({'time': 'sum'})
train_uesr1
...
class RecommenderNet(nn.Layer):
    ...
    def forward(self, data):
        user, goods, feat = data[0], data[1], [data[idx] for idx in [2, 3, 4, 5]]
...
optimizer = paddle.optimizer.Adam(parameters=model.parameters(), learning_rate=0.000025)
